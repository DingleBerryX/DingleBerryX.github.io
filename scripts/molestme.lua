...
local function checkForModerators()
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if table.find(moderatorIds, player.UserId) then
            detected = true
            library:Notify("Moderator detected: " .. player.Name .. " (ID: " .. player.UserId .. ")", 5, "alert")
        end
    end
    if detected then
...
if _G.isRapehookLoaded then
    for _, connection in pairs(_G.RapehookConnections or {}) do
...
local function isTeamMate(player)
    return not ffaMode and player.Team and player.Team == LocalPlayer.Team
end
...
AimbotTab:NewSlider("FOV Size", "px", true, "", {min = 50, max = 500, default = 100, step = 1}, function(v)
    fovSize = v
    fovDrawing.Radius = v
    updateConfig()
end)
AimbotTab:NewSlider("Smoothing", "%", true, "", {min = 0, max = 1, default = 0.2, step = 0.01}, function(v)
    aimbotSmoothing = v
    updateConfig()
end)
AimbotTab:NewSlider("Hit Chance", "%", true, "", {min = 0, max = 100, default = 100, step = 1}, function(v)
    hitChance = v
    updateConfig()
end)
...
VisualsTab:NewSelector("Viewmodel Material", "Plastic", materials, function(v)
    viewmodelMaterial = Enum.Material[v]
    updateViewmodel()
    updateConfig()
end)
VisualsTab:NewButton("Cycle Weapon Color", function()
    viewmodelColor = Color3.fromRGB(math.random(0, 255), math.random(0, 255), math.random(0, 255))
    updateViewmodel()
    updateConfig()
end)
VisualsTab:NewButton("Cycle Arm Color", function()
...
MiscTab:NewButton("Anticheat Disabler", function()
    Library:Notify("Anticheat disabler not implemented yet", 3, "info")
end)
MiscTab:NewToggle("FFA Mode", false, function(v)
    ffaMode = v
    updateConfig()
end)
...
ConfigTab:NewButton("Save Config", function()
    ...
    if writefile then
        ...
        Library:Notify("Config saved successfully", 3, "success")
    else
        setclipboard(configString)
        Library:Notify("Config copied to clipboard", 3, "info")
    end
end)
ConfigTab:NewButton("Load Config", function()
    ...
    if configString then
        ...
        if success then
            ...
            fovSize = config.fovSize or 100
            aimbotSmoothing = config.aimbotSmoothing or 0.2
            ...
            flyBind = Enum.KeyCode[config.flyBind] or Enum.KeyCode.V
            hitChance = config.hitChance or 100
            fovEnabled = config.fovEnabled or false
            silentAimEnabled = config.silentAimEnabled or false
            viewmodelMaterial = Enum.Material[config.viewmodelMaterial] or Enum.Material.Plastic
            viewmodelColor = Color3.fromRGB(unpack(config.viewmodelColor or {255, 255, 255}))
            armColor = Color3.fromRGB(unpack(config.armColor or {255, 255, 255}))
            espColor = Color3.fromRGB(unpack(config.espColor or {0, 255, 0}))
            skeletonColor = Color3.fromRGB(unpack(config.skeletonColor or {0, 255, 0}))
            chamsFillColor = Color3.fromRGB(unpack(config.chamsFillColor or {0, 255, 0}))
            chamsOutlineColor = Color3.fromRGB(unpack(config.chamsOutlineColor or {0, 0, 0}))
            chamsMaterial = Enum.Material[config.chamsMaterial] or Enum.Material.Plastic
            fovDrawing.Radius = fovSize
            updateViewmodel()
