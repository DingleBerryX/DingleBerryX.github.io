...
local thirdPersonBind = Enum.KeyCode.P
...
local function updateConfig()
    config = {
        aimbotEnabled = aimbotEnabled, fovSize = fovSize, aimbotSmoothing = aimbotSmoothing,
        espEnabled = espEnabled, skeletonEnabled = skeletonEnabled, chamsEnabled = chamsEnabled,
        flyBind = flyBind.Name, hitChance = hitChance, fovEnabled = fovEnabled,
        silentAimEnabled = silentAimEnabled, thirdPersonEnabled = thirdPersonEnabled,
        thirdPersonBind = thirdPersonBind.Name, viewmodelMaterial = tostring(viewmodelMaterial),
...
local function createBoxESP(player)
    ...
    local espGroup = {
        BoxOuter = Drawing.new("Square"), BoxInner = Drawing.new("Square"),
        NameTag = Drawing.new("Text"), HealthBar = Drawing.new("Square"),
        HealthFill = Drawing.new("Square")
    }
    for _, drawing in pairs(espGroup) do drawing.Visible = false drawing.Transparency = 1 end
    espGroup.BoxOuter.Thickness = 2 espGroup.BoxOuter.Color = Color3.fromRGB(0, 0, 0) espGroup.BoxOuter.Filled = false
    espGroup.BoxInner.Thickness = 1 espGroup.BoxInner.Color = espColor espGroup.BoxInner.Filled = false
    espGroup.NameTag.Color = Color3.fromRGB(255, 255, 255) espGroup.NameTag.Size = 14 espGroup.NameTag.Center = true espGroup.NameTag.Outline = true espGroup.NameTag.OutlineColor = Color3.fromRGB(0, 0, 0)
    espGroup.HealthBar.Thickness = 1 espGroup.HealthBar.Color = Color3.fromRGB(0, 0, 0) espGroup.HealthBar.Filled = true
    espGroup.HealthFill.Color = espColor espGroup.HealthFill.Filled = true

    local function updateESP()
        espGroup.BoxInner.Color = espColor espGroup.HealthFill.Color = espColor
        if not espEnabled or isTeamMate(player) or not player.Character or not player.Character.Parent then
            for _, drawing in pairs(espGroup) do drawing.Visible = false end return
        end
        ...
        if onScreen then
            ...
            local boxSize = Vector2.new(width, height) local boxPos = Vector2.new(vector.X - width/2, vector.Y - height/2)
            espGroup.BoxOuter.Size = boxSize + Vector2.new(4, 4) espGroup.BoxOuter.Position = boxPos - Vector2.new(2, 2)
            espGroup.BoxInner.Size = boxSize espGroup.BoxInner.Position = boxPos
            espGroup.NameTag.Position = Vector2.new(vector.X, boxPos.Y - 16) espGroup.NameTag.Text = player.Name
            local healthPercent = humanoid.Health / humanoid.MaxHealth
            espGroup.HealthBar.Size = Vector2.new(4, height + 4) espGroup.HealthBar.Position = Vector2.new(boxPos.X - 8, boxPos.Y - 2)
    ...
    player.CharacterRemoving:Connect(function() for _, drawing in pairs(espGroup) do drawing:Remove() end end)
end
...
local function createSkeletonESP(player)
    ...
    for _, pair in pairs(skeletonParts) do
        ...
        line.Thickness = 1 line.Color = skeletonColor line.Visible = false line.Transparency = 1
    ...
    local function updateSkeleton()
        if not skeletonEnabled or isTeamMate(player) or not player.Character or not player.Character.Parent then
            for _, line in pairs(skeletonLines) do line.Visible = false end return
        end
        local humanoid = player.Character:FindFirstChild("Humanoid")
    ...
    player.CharacterRemoving:Connect(function() for _, line in pairs(skeletonLines) do line:Remove() end end)
end

local function createChams(player)
    if isTeamMate(player) then return end
    local highlight = Instance.new("Highlight")
    highlight.Name = "Chams" highlight.FillColor = chamsFillColor highlight.OutlineColor = chamsOutlineColor
    highlight.FillTransparency = 0.5 highlight.OutlineTransparency = 0 highlight.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
    highlight.Enabled = false highlight.Adornee = player.Character highlight.Parent = player.Character
    local function updateChams()
        highlight.FillColor = chamsFillColor highlight.OutlineColor = chamsOutlineColor
        highlight.Enabled = chamsEnabled and player.Character and not isTeamMate(player)
    end
...
fovDrawing.Thickness = 1 fovDrawing.NumSides = 100 fovDrawing.Radius = fovSize fovDrawing.Filled = false
fovDrawing.Visible = false fovDrawing.Transparency = 1 fovDrawing.Color = fovColor
...
local function aimbotUpdate()
    ...
    fovDrawing.Radius = fovSize fovDrawing.Color = fovColor fovDrawing.Visible = fovEnabled and aimbotEnabled
...
local flying = false local noclipping = false local speed = 50
local bodyVelocity, bodyGyro, noclipConnection

local function startFlying()
    if flying then return end flying = true
    bodyVelocity = Instance.new("BodyVelocity") bodyVelocity.Velocity = Vector3.new(0,0,0)
    bodyVelocity.MaxForce = Vector3.new(math.huge, math.huge, math.huge) bodyVelocity.Parent = rootPart
    bodyGyro = Instance.new("BodyGyro") bodyGyro.P = 9e4 bodyGyro.MaxTorque = Vector3.new(9e9, 9e9, 9e9)
    bodyGyro.CFrame = rootPart.CFrame bodyGyro.Parent = rootPart humanoid.PlatformStand = true
    spawn(function()
        while flying do
            local cam = Workspace.CurrentCamera local moveDirection = Vector3.new(0,0,0)
            ...
            bodyGyro.CFrame = cam.CFrame wait()
        end
    end)
end
...
local function startNoclip()
    if noclipping then return end noclipping = true
...
local function stopNoclip()
    if not noclipping then return end noclipping = false
    if noclipConnection then noclipConnection:Disconnect() end
    for _, part in pairs(character:GetDescendants()) do if part:IsA("BasePart") and part.Name ~= "HumanoidRootPart" then part.CanCollide = true end end
end

local function toggleFlightAndNoclip()
    if flying or noclipping then stopFlying() stopNoclip() else startFlying() startNoclip() end
end
...
local function toggleThirdPerson()
    ...
    if thirdPersonEnabled then
        ...
        spawn(function()
            while thirdPersonEnabled and humanoid and rootPart do
                Camera.CFrame = CFrame.new(rootPart.Position + Vector3.new(5, 5, 5), rootPart.Position)
                wait()
            end
            if not thirdPersonEnabled then Camera.CameraType = Enum.CameraType.Custom Camera.CameraSubject = humanoid end
        end)
    else
    ...
    if viewmodel then
        for _, part in pairs(viewmodel:GetDescendants()) do
            if part:IsA("BasePart") then
                if part.Name:lower():find("arm") then part.Color = armColor
                else part.Material = viewmodelMaterial part.Color = viewmodelColor end
            end
        end
    end
end
...
AimbotTab:NewToggle("Silent Aim", false, function(v) silentAimEnabled = v updateConfig() end)
AimbotTab:NewSlider("FOV Size", "", true, "", {min = 50, max = 500, default = 100}, function(v) fovSize = v fovDrawing.Radius = v updateConfig() end)
AimbotTab:NewSlider("Smoothing", "", true, "", {min = 0.1, max = 1, default = 0.2}, function(v) aimbotSmoothing = v updateConfig() end)
AimbotTab:NewSlider("Hit Chance", "", true, "%", {min = 0, max = 100, default = 100}, function(v) hitChance = v updateConfig() end)
...
VisualsTab:NewToggle("Box ESP", false, function(v) espEnabled = v updateConfig() end)
...
VisualsTab:NewToggle("Skeleton ESP", false, function(v) skeletonEnabled = v updateConfig() end)
...
VisualsTab:NewToggle("Chams", false, function(v) chamsEnabled = v updateConfig() end)
...
MiscTab:NewKeybind("Third Person Bind", Enum.KeyCode.P, function(k) 
    thirdPersonBind = Enum.KeyCode[k] 
    updateConfig()
end)
MiscTab:NewToggle("Third Person", false, function(v) 
...
ConfigTab:NewButton("Save Config", function()
    ...
    if writefile then writefile("rapehook_config.json", configString) else setclipboard(configString) library:Notify("Config copied to clipboard", 3) end
end)
ConfigTab:NewButton("Load Config", function()
    ...
    if configString then
        ...
        if success then
            config = decoded
            aimbotEnabled = config.aimbotEnabled fovSize = config.fovSize aimbotSmoothing = config.aimbotSmoothing
            espEnabled = config.espEnabled skeletonEnabled = config.skeletonEnabled chamsEnabled = config.chamsEnabled
            flyBind = Enum.KeyCode[config.flyBind] hitChance = config.hitChance fovEnabled = config.fovEnabled
            silentAimEnabled = config.silentAimEnabled thirdPersonEnabled = config.thirdPersonEnabled
            thirdPersonBind = Enum.KeyCode[config.thirdPersonBind] viewmodelMaterial = Enum.Material[config.viewmodelMaterial]
            viewmodelColor = Color3.fromRGB(unpack(config.viewmodelColor)) armColor = Color3.fromRGB(unpack(config.armColor))
            espColor = Color3.fromRGB(unpack(config.espColor)) skeletonColor = Color3.fromRGB(unpack(config.skeletonColor))
            chamsFillColor = Color3.fromRGB(unpack(config.chamsFillColor)) chamsOutlineColor = Color3.fromRGB(unpack(config.chamsOutlineColor))
            chamsMaterial = Enum.Material[config.chamsMaterial] fovDrawing.Radius = fovSize
...
table.insert(_G.rapehookConnections, UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if input.KeyCode == flyBind then toggleFlightAndNoclip()
    elseif input.KeyCode == thirdPersonBind and not gameProcessed then toggleThirdPerson() end
end))

for _, player in pairs(Players:GetPlayers()) do
    if player ~= LocalPlayer then 
        createBoxESP(player) createSkeletonESP(player) createChams(player)
    end
end
...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    if flying or noclipping then stopFlying() stopNoclip() wait(0.1) if flying then startFlying() end if noclipping then startNoclip() end end
    if thirdPersonEnabled then toggleThirdPerson() end
end)
